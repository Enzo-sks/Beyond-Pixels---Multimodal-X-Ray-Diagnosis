...
class BidirectionalCrossAttention(nn.Module):
    ...
    Args:
        ...
        use_i2t   : si False, désactive la direction Image→Texte (image queries
                    × text keys/values). C'est cette direction qui injecte le
                    contenu du rapport dans la représentation image — la
                    désactiver élimine le canal architectural responsable du
                    text leakage identifié dans l'analyse d'interprétabilité
                    (cf. interpretability/mediation_auc.py). Le texte continue
                    d'être enrichi par l'image (t2i reste actif).
    """
    ...
    def forward(
        ...
        # (avant mise à jour) pour un flux d'information simultané dans les
        # deux directions.
        ...
        if self.use_i2t:
            ...
            # Pas d'attention Image→Texte : I reste purement visuel, seul le
            # résidu (sans terme cross-attention) passe par la LayerNorm.
            I = self.norm_i1(I)
...
class MultimodalFusion(nn.Module):
    ...
    def __init__(
        ...
        self.cross_attn = BidirectionalCrossAttention(
            ...
            d_model=d_model, n_heads=n_heads, dropout=dropout, use_i2t=use_i2t,
        )
